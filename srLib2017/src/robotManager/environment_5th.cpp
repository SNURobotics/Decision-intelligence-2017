...
void Tape::AssembleModel()
{
	...
	//m_ObjLink[0].GetGeomInfo().SetLocalFrame(EulerZYX(Vec3(-SR_PI_HALF, 0.0, SR_PI_HALF), Vec3(-0.01675, 0.0, -0.004)));
	//m_ObjLink[0].GetGeomInfo().SetLocalFrame(EulerZYX(Vec3(0.0, 0.0, 0.0), Vec3(-0.10007, 0.09307, 0.0888)));
	m_ObjLink[0].GetGeomInfo().SetLocalFrame(EulerZYX(Vec3(0.0, 0.0, 0.0), Vec3(0.0, 0.0, 0.0))); // SetLocalFrame --> Orientation: SolidworkssrLib���� solidworks�� ���� SO3, Position: srLib frame�� orientation �������� ���� �̵��ϰ� ���� ��ŭ �Է�
...
ACB_FU::ACB_FU(double collision_offset /*= 0.01*/)
{
	m_collision_offset = collision_offset;
	AssembleModel();
}
...
void ACB_FU::AssembleModel()
{
	...
	m_ObjLink[0].GetGeomInfo().SetShape(srGeometryInfo::TDS);
	m_ObjLink[0].GetGeomInfo().SetFileName("../../../workspace/robot/objects_3ds/ACB01-FU.3ds");
	m_ObjLink[0].GetGeomInfo().SetLocalFrame(EulerZYX(Vec3(SR_PI_HALF, 0, 0), Vec3(0, 0, 0)) * EulerZYX(Vec3(0, 0, 0), Vec3(-0.23, 0, 0)));
	// xy plane
	m_ObjWeldJoint[0].SetParentLink(&m_ObjLink[0]);
	m_ObjWeldJoint[0].SetParentLinkFrame();
	m_ObjWeldJoint[0].SetChildLink(&m_ObjLink[1]);
...
ACB_FL::ACB_FL(double collision_offset /*= 0.01*/)
{
	m_collision_offset = collision_offset;
	AssembleModel();
}
...
void ACB_FL::AssembleModel()
{
	...
	m_ObjLink[0].GetGeomInfo().SetShape(srGeometryInfo::TDS);
	m_ObjLink[0].GetGeomInfo().SetFileName("../../../workspace/robot/objects_3ds/ACB01-FL.3ds");
	m_ObjLink[0].GetGeomInfo().SetLocalFrame(EulerZYX(Vec3(SR_PI_HALF, 0, 0), Vec3(0,0,0)) * EulerZYX(Vec3(0, 0, -SR_PI_HALF), Vec3(-0.23, 0, 0.17)));
	// xy plane
	m_ObjWeldJoint[0].SetParentLink(&m_ObjLink[0]);
	m_ObjWeldJoint[0].SetParentLinkFrame();
	m_ObjWeldJoint[0].SetChildLink(&m_ObjLink[1]);
