...
static Eigen::MatrixXd pinv(const Eigen::MatrixXd& _matrix)
{
	...
	for (int i = 0; i < min(_matrix.cols(), _matrix.rows()); i++)
	{
...
static Eigen::VectorXd VecToVector(const vector<double>& vec)
{
...
static vector<double> VectorToVec(const Eigen::VectorXd& vect)
{
...
static vector<double> SO3ToEulerZYX(const SO3& R)
{
...
static vector<double> SO3ToEulerXYZ(const SO3& R)
{
	vector<double> euler(3);
	euler[1] = asin(R[6]);
...
static SE3 SKKUtoSE3(const vector<double>& ori, const vector<double>& pos)
{
