...
void srSystem::UpdateCOMZMP(Vec3& g)
{
	...
	double totalMass = 0.0;	// total mass
	double totalMoment = 0.0;	// total moment
	Vec3 mc(0.0);	// total mass * mass center position
	Vec3 ma(0.0);	// total mass * mass center acceleration

	double lm;	// each link mass
...
void srSystem::DIFFKIN_IntegratePosition(double & _step)
{
	(this->*m_pfn_diffkin_integrateposition)(_step);
}

void srSystem::__L_IntegratePosition_Euler(double & _step)
{
	if (m_BaseLinkType != FIXED)
		m_L_Link->m_Frame *= Exp(_step * m_L_Link->m_Vel);
}

void srSystem::__FS_IntegratePosition_Euler(double & _step)
{
	int i, iCount;
	srLink * pMass;
...
void srSystem::DYN_ForwardDynamics_Set00(double & _step)
{
	(this->*m_pfn_dyn_forwarddynamics_set00)(_step);
}

void srSystem::_FS_ForwardDynamics_Set00(double & _step)
{
...
void srSystem::_L_ForwardDynamics_Set00(double & _step)
{
	//__L_UpdateBiasforce();
...
void srSystem::__FS_IntegrateVelocity_Euler(double & _step)
{
	int i, iCount;
	srLink * pMass;
...
void srSystem::__L_IntegrateVelocity_Euler(double & _step)
{
	if (m_BaseLinkType != FIXED)
		m_L_Link->m_Vel += _step * m_L_Link->m_Acc;
}

void srSystem::DYN_ImpulseDynamics_Set00(double & _fps)
{
	(this->*m_pfn_dyn_impulsedynamics_set00)(_fps);
}

void srSystem::_FS_Impulsedynamics_set00(double & _fps)
{
...
void srSystem::_L_Impulsedynamics_set00(double & _fps)
{
	//L_UpdateDelVelocity();
...
void srSystem::__FS_UpdateDelVelocity(double & _fps)
{
	int i, iCount;
	se3	DV;
	srLink * pMass;
...
void srSystem::__FS_UpdateAcceleration_with_DelVelocity(double & _fps)
{
	int i, iCount;
	srLink * pMass;
	srState * pState;
...
void srSystem::__L_UpdateAcceleration_with_DelVelocity(double & _fps)
{
	if (m_BaseLinkType == DYNAMIC)
		m_L_Link->m_Acc += (_fps * m_L_Link->m_DelVel);
}

void srSystem::DYN_IntegratePosition_Set00(double & _step)
{
	(this->*m_pfn_dyn_integrateposition_set00)(_step);
}

void srSystem::_FS_IntegratePosition_Set00(double & _step)
{
	...
	if (m_BaseLinkType != FIXED){
		//pMass->m_Frame *= Exp(_step * pMass->m_Vel);
		SO3 R = pMass->m_Frame.GetOrientation();
		Vec3 w(pMass->m_Vel[0], pMass->m_Vel[1], pMass->m_Vel[2]);
		Vec3 v(pMass->m_Vel[3], pMass->m_Vel[4], pMass->m_Vel[5]);
		SE3 T(R*Exp(_step * w), pMass->m_Frame.GetPosition() + R*v*_step);
		pMass->m_Frame = T;
	}
...
void srSystem::_L_IntegratePosition_Set00(double & _step)
{
	//DIFFKIN_IntegratePosition(_step);
	//DIFFKIN_LinkVelocityPropagation();
...
void srSystem::FS_IntegratePosition_PosErrVel_Euler(double & _step)
{
	int i, iCount;
	srLink * pMass;
...
void srSystem::FS_IntegratePosition_PosErrVel_plus_Velocity_Euler(double & _step)
{
	int i, iCount;
	srLink * pMass;
