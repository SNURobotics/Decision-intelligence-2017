...
void srSpace::SetGravity(double x, double y, double z)
{
	m_Gravity = Vec3(x,y,z);
}

void srSpace::SetTimestep(double v)
{
	if ( v > 0.0)
...
const double srSpace::GetSimulationTime() const
{
...
bool srSpace::_KIN_COLLISION_RUNTIME_SIMULATION_LOOP()
{
	bool occurCollision = false;
	for (int i = 0; i < m_nSubstep_render_fixed; i++)
	{
		_KIN_UpdateFrame_All_The_Entity_All_The_Systems();
		if (m_srDYN.RUNTIME_MARKKH())
			occurCollision = true;
		_Step_Simulation_Time();
	}
