...
bool	LCPSolver_SPARSE_PGS____MARK1(int n, int * r, int * c, int * rowofs, double * A,
									  double * x, double * b, double * lo, double * hi,
...
bool	LCPSolver_PGS____MARK3(int n, int nskip, int nub, double * A, double * x, double * b, 
...
bool	LCPSolver_PGS____MARK4(int n, int nskip, int /*nub*/, double * A, double * x, double * b, 
							   double * lo, double * hi, int * findex, PGSOption * option)
{
	// LDLT solver will work !!!
	//if (nub == n)
...
void ISLAND::_PGS_Solve_Constraint_____________________MARK7(PGSOption * option)
{
	...
	ALLOC_BK(double, lo, nt);			// lo : lower bound of lambda
	ALLOC_BK(double, hi, nt);			// hi : upper bound of lambda
	ALLOC_BK(int, findex, nt);			// findex : friction index
	ALLOC_BK(double, rhs,nt);			// rhs : c - J(v_old + invM*f_ext*h) where c is max( penet/h , -e*J*v_old )
	ALLOC_BK(double, lambda, nt);		// lambda : constraint force
	ALLOC_BK(double, invMc, j);			// ivMc : Constraint inverse mass matrix ( J*invM*Jt )
