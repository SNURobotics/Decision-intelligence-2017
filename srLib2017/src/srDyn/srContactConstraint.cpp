...
double ContactConstraint::erp_contact = 0.8;
double ContactConstraint::allowedpenetration = 0.01;
double ContactConstraint::bouncing_threshold = 0.2;
double ContactConstraint::maximum_erp_velocity = 10;
double ContactConstraint::maximum_bouncing_velocity = 30;


void ContactConstraint::SetErp(double _erp)
{
	erp_contact = _erp;
}

void ContactConstraint::SetAllowedPenetration(double _allowedpenetration)
{
	allowedpenetration = _allowedpenetration;
}

void ContactConstraint::SetBouncingThreshold(double _bouncingthreshold)
{
	bouncing_threshold = _bouncingthreshold;
}

void ContactConstraint::SetMaximumErpVelocity(double _maximum_erp_velocity)
{
	maximum_erp_velocity = _maximum_erp_velocity;
}

void ContactConstraint::SetMaximumBouncingVelocity(double _maximum_bouncing_velocity)
{
...
double	ContactConstraint::_PRESTEP_Calculate_FrictionCoeff___________MARK7(double leftmat, double rightmat)
{
	return min(leftmat, rightmat);
}

double	ContactConstraint::_PRESTEP_Calculate_RestitutionCoeff________MARK7(double leftmat, double rightmat)
{
	//return min(leftmat, rightmat);
...
void			ContactConstraint::HowAboutThisPoint(Vec3& point , Vec3& normal , double & penetration)
{
...
inline int ContactConstraint::SortContactPoints(Vec3& localpoint_Left, double & penetration)
{
	...
	double area_n = SquareSum(Cross(a10,a20));
	double area_1 = SquareSum(Cross(an0,a20));
	double area_2 = SquareSum(Cross(an0,a10));
	double area_0 = SquareSum(Cross(a21,an1));


	int insertIdx = -1;
	double maxarea = area_n;
	if (area_1 > maxarea)
	{
		insertIdx = 1;
		maxarea = area_1;
...
{
	double dist2 = CONTACT_THRESHOLD_SQR;
	double dist1;
...
inline void ContactConstraint::InsertContactPointAt(Vec3 & point, Vec3 & normal, double & penetration, Vec3& localpoint_Left, int insertIdx)
{
...
void ContactConstraint::GetInformation(ConstraintInfo * info)
{
	...
	if (bFriction)
	{
		...
		for (i = 0 ; i < nContactPts ; i++)
		{
			...
			double c = pContactPts[i]->Penetration - allowedpenetration;
			if (c < 0.0)
			{
				c = 0.0;
			}
			...
			double & neg_relvel = info->_rhs[tmp];
			...
			if (bBounce)
			{
				...
				{
					double newc = rRestitution_coeff * neg_relvel;
...
inline void ContactConstraint::__getdelvelocity_Leftside(double *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
inline void ContactConstraint::__getdelvelocity_Rightside(double *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
inline void ContactConstraint::__getdelvelocity_Bothsides(double *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
inline void ContactConstraint::__getdelvelocity_None(double *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
		sjari[i] = 0.0;
...
void ContactConstraint::_getdelvelocity_BothMove(double *sjari)
{
...
void ContactConstraint::_getdelvelocity_LeftMove(double *sjari)
{
	if (pLSystem->m_bExcited)
		__getdelvelocity_Leftside(sjari);
	else
...
void ContactConstraint::_getdelvelocity_RightMove(double *sjari)
{
	if (pRSystem->m_bExcited)
		__getdelvelocity_Rightside(sjari);
	else
...
void ContactConstraint::GetDelVelocity(double * sjari)
{
...
void ContactConstraint::_getrelvelocity_LeftMove(double * sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
void ContactConstraint::_getrelvelocity_RightMove(double * sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
void ContactConstraint::_getrelvelocity_BothMove(double * sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
void ContactConstraint::GetRelVelocity(double * sjari)
{
	(this->*m_pfn_getrelvelocity)(sjari);
}

void ContactConstraint::SetImpulse(double * _lambda)
{
	int i;
	if (bFriction)
	{
