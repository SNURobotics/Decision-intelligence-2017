...
double ClosedLoop::erp_closedloop = 0.8;
double ClosedLoop::allowederror = 0.01;
double ClosedLoop::maximum_erp_velocity = 10;

void ClosedLoop::SetErp(double _erp)
{
	erp_closedloop = _erp;
}

void ClosedLoop::SetAllowedPenetration(double _allowedpenetration)
{
	allowederror = _allowedpenetration;
}

void ClosedLoop::SetMaximumErpVelocity(double _maximum_erp_velocity)
{
	maximum_erp_velocity = _maximum_erp_velocity;
}

inline void ClosedLoop::GetError(double _recip_timestep)
{
...
void ClosedLoop::GetInformation(ConstraintInfo * info)
{
	...
	memcpy(info->_lambda,lambda,6*sizeof(double));
}
...
void ClosedLoop::GetDelVelocity(double * sjari)
{
	if (pSystem->m_bExcited)
	{
		// Need operator like (double* = (se3&))
...
void ClosedLoop::SetImpulse(double * _lambda)
{
	memcpy(lambda,_lambda,6*sizeof(double));
