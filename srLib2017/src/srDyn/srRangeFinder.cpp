...
: srSensor()
{
	...
	m_NumSpots = (int)((double)m_Spread / m_Resolution) + 1;

	m_SpreadRad = DEG2RAD((double)m_Spread * 0.5);
...
double srRangeFinder::GetMaxRange()
{
	return m_MaxRange;
}

double srRangeFinder::GetMinRange()
{
	return m_MinRange;
}

void srRangeFinder::SetMaxRange(double max)
{
	if(max > m_MinRange) {
		m_MaxRange = max;
	}
...
void srRangeFinder::SetMinRange(double min)
{
...
void srRangeFinder::SetRange(double max, double min)
{
	if(max > min && min >= 0.0) {
		m_MaxRange = max;
		m_MinRange = min;
...
double srRangeFinder::GetResolution()
{
	return m_Resolution;
}
void srRangeFinder::SetResolution(double r)
{
	if(0.0 < r && r < m_Spread)
	{
		m_Resolution = r;
...
double* srRangeFinder::GetDetectedValue()
{
