...
double& srCollision::GetBoundingRadius()
{
...
//-- Ray Detection
bool srCollision::RayDetection(Vec3& point, Vec3& direction, double& range, double& dist)
{
...
//-- Touch Detection
bool srCollision::TouchDetection(Vec3& position, double radius, Vec3& point, Vec3& normal, double& penetration)
{
...
//-- Sonar Detection
bool srCollision::SonarDetection(SE3& T0, double & range , double & angle, bool & res)
{
...
bool RayToSphere(Vec3& point,Vec3& direction, double& range, 
				 const SE3 &T, double r,
				 double& dist)
{
	Vec3 q(&T[9]);
...
bool RayToCylinder(Vec3& point, Vec3& direction, double& range, 
				   const SE3 &T, double r, double h,
				   double& dist)
{
...
void Inv3by3(double* x, double* b)	// x * b = I
{
	double a11 = x[4]*x[8]-x[5]*x[7];
	double a12 = x[1]*x[8]-x[2]*x[7];
	double a13 = x[1]*x[5]-x[2]*x[4];
	double det = x[0]*a11-x[3]*a12+x[6]*a13;
	det = 1.0/det;
...
void Mult3by3(double* a, double* b, double* r)	// a*b=r, a,b,r: 3 by 3 matrix
{
...
void MultMV3by3(double*a, double*v, double*r)		// a*v=r, a: 3 by 3 matrix, v: 3-dim vector, r:3-dim vector
{
...
bool srCollision::_RayToBox(Vec3& point, Vec3& direction, double& range, double& dist)
{
	SE3 & T = GetFrame();
	...
	static double A[9];						//static RMatrix A(3,3);
	static double InvA[9];					//inverse matrix of A
	Vec3 q1(&T[0]),q2(&T[3]),q3(&T[6]);
	q1 *= size[0];
	q2 *= size[1];
	q3 *= size[2];

	memcpy(&A[0],&q1[0],sizeof(double)*3);	//memcpy(&A(0,0),&q1[0],sizeof(double)*3);
	memcpy(&A[3],&q2[0],sizeof(double)*3);	//memcpy(&A(0,1),&q2[0],sizeof(double)*3);
	memcpy(&A[6],&q3[0],sizeof(double)*3);	//memcpy(&A(0,2),&q3[0],sizeof(double)*3);

	static double B[3];						//static RMatrix B(3,1);
	memcpy(&B[0],&p1[0],sizeof(double)*3);	//memcpy(&B(0,0),&p1[0],sizeof(double)*3);
	...
	static double PC[3];						//static RMatrix C(3,1);
	static double C[3];						// InvA*PC=C;
	memcpy(&PC[0],&v[0],sizeof(double)*3);	//memcpy(&C(0,0),&v[0],sizeof(double)*3);

	Inv3by3(A, InvA);						//A = Inv(A);

	static double Z[3];						//static RMatrix Z(3,1);
											// InvA*B=Z
	...
	double minGlobal = -10000, maxGlobal = 10000;
	for(int i=0;i<3;i++){
		if(Z[i] > 0){								//if(Z(i,0) > 0){
...
bool srCollision::_RayToSphere(Vec3& point, Vec3& direction, double& range, double& dist)
{
	SE3 & T = GetFrame();
	double r = 0.5*GetDimension()[0];
	//
...
bool srCollision::_RayToCylinder(Vec3& point, Vec3& direction, double& range, double& dist)
{
	SE3 & T = GetFrame();
	double r = 0.5*GetDimension()[0];
	double h = GetDimension()[1];
	//
...
bool srCollision::_RayToCapsule(Vec3& point, Vec3& direction, double& range, double& dist)
{
	SE3 & T = GetFrame();
	double r = 0.5*GetDimension()[0];
	double h = GetDimension()[1];
...
bool srCollision::_RayToPlane(Vec3& point, Vec3& direction, double& range, double& dist)
{
	SE3 & T = GetFrame();
	//
...
bool srCollision::_RayToUser(Vec3& /*point*/, Vec3& /*direction*/, double& /*range*/, double& /*dist*/)
{
	return false;
}
...
bool srCollision::_TouchToBox(Vec3& c0, double r0, Vec3& point, Vec3& normal, double& penetration)
{
	// Sphere
...
bool srCollision::_TouchToSphere(Vec3& c0, double r0, Vec3& point, Vec3& normal, double& penetration)
{
	// Sphere1

	// Sphere2
	SE3 & c1 = GetFrame();
	double r1 = 0.5*GetDimension()[0];
...
bool srCollision::_TouchToCylinder(Vec3& /*position*/, double /*radius*/, Vec3& /*point*/, Vec3& /*normal*/, double& /*penetration*/)
{
	// 미구현
	return false;
}

bool srCollision::_TouchToCapsule(Vec3& c1, double r1, Vec3& point, Vec3& normal, double& penetration)
{
	...
	// Capsule
	SE3 & T0 = GetFrame();
	double r0 = 0.5*GetDimension()[0];
	double h0 = GetDimension()[1];
...
bool srCollision::_TouchToPlane(Vec3& c1, double r1, Vec3& point, Vec3& normal, double& penetration)
{
	// Plane
	SE3 & T0 = GetFrame();
...
bool srCollision::_TouchToUser(Vec3& /*position*/, double /*radius*/, Vec3& /*point*/, Vec3& /*normal*/, double& /*penetration*/)
{
	return false;
}
...
========================================================================== */
bool _ConeToSlab(Vec3& normal, Vec3& center, double min, double max, SE3& T0, double& range, double& radius)
{
	Vec3 Q;
	Vec3 m;
	...
	double A = Inner(normal, P);
	// project Q onto normal
	double B = Inner(normal, Q);
...
bool srCollision::_SonarToBox(SE3& T0, double & range , double & angle, bool & res)
{
	// box information
	SE3& Tbox = GetFrame();
	Vec3 dim = 0.5*GetDimension();
	double radius = range * tan(angle * 0.5);
...
bool srCollision::_SonarToSphere(SE3& T0, double & range , double & angle, bool & res)
{
	// Sphere
	SE3 & c = GetFrame();
	double r = 0.5*GetDimension()[0];
...
//		ASSERT(abs(intercept[1]) < 10e-6 && abs(intercept[2]) < 10e-6);
		if(intercept[0] <= 0.0) {	// intercept is above the apex
			...
			double s = intercept[0] * sin(angle * 0.5);
...
bool srCollision::_SonarToCapsule(SE3& /*T0*/, double & /*range*/, double & /*angle*/, bool & /*res*/)
{
	return false;
}

bool srCollision::_SonarToCylinder(SE3& /*T0*/, double & /*range*/, double & /*angle*/, bool & /*res*/)
{
	return false;
}

bool srCollision::_SonarToPlane(SE3& T0, double & range , double & angle, bool & res)
{
	// Plane
	SE3 & T1 = GetFrame();
...
bool srCollision::_SonarToUser(SE3& /*T0*/, double & /*range*/, double & /*angle*/, bool & /*res*/)
{
	return false;
}
