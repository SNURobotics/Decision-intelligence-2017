...
double& srIRSensor::GetDetectedValue()
{
	return m_DetectedValue;
}

double srIRSensor::GetMaxRange()
{
	return m_MaxRange;
}

double srIRSensor::GetMinRange()
{
	return m_MinRange;
}

void srIRSensor::SetMaxRange(double max)
{
	if(max > m_MinRange) {
		m_MaxRange = max;
	}
...
void srIRSensor::SetMinRange(double min)
{
...
void srIRSensor::SetRange(double max, double min)
{
	if(max > min && min >= 0.0) {
		m_MaxRange = max;
		m_MinRange = min;
