...
simxFloat* Vec3ToSimxFloat(Vec3 vec);
...
int main(int argc,char* argv[])
{
	...
	box1 = createBox(EulerZYX(Vec3(0.3, 0.1, 0.2), Vec3(0.0, 0.0, 1.5)), Vec3(0.1, 0.1, 0.1));
	box2 = createBox(EulerZYX(Vec3(0.0, -0.1, 0.2), Vec3(0.05, 0.05, 0.7)), Vec3(0.1, 0.1, 0.1));
	box3 = createBox(EulerZYX(Vec3(0.1, 0.1, -0.2), Vec3(0.05, -0.05, 1.1)), Vec3(0.1, 0.1, 0.1));
	//box0 = createBox(SE3(Vec3(0.0, 0.0, -0.005)), Vec3(10.0, 10.0, 0.01), Vec3(0.1, 0.1, 0.1), srSystem::BASELINKTYPE::FIXED);
	...
	if (clientID != -1)
	{
		...
		//pos[1] = 0.5;
		//pos[2] = 0.5;
		//simxSetObjectPosition(clientID, obj1handle, -1, pos, simx_opmode_blocking);
		//T1 = SE3toVec3Pair(box1->GetBaseLink()->GetFrame());
		//for (int i = 0; i < 3; i++)
		//	pos[i] = T1.first[i];
		//simxSetObjectPosition(clientID, obj2handle, -1, pos, simx_opmode_blocking);
...
void setVrepFromSrlib()
{
	...
	//int check = simxSetObjectPosition(clientID, obj1handle, -1, Vec3ToSimxFloat(T1.first), simx_opmode_blocking);
	//check = simxSetObjectPosition(clientID, obj2handle, -1, Vec3ToSimxFloat(T2.first), simx_opmode_blocking);
	//check = simxSetObjectPosition(clientID, obj3handle, -1, Vec3ToSimxFloat(T3.first), simx_opmode_blocking);
	//check = simxSetObjectOrientation(clientID, obj1handle, -1, Vec3ToSimxFloat(T1.second), simx_opmode_blocking);
	//check = simxSetObjectOrientation(clientID, obj2handle, -1, Vec3ToSimxFloat(T2.second), simx_opmode_blocking);
	//check = simxSetObjectOrientation(clientID, obj3handle, -1, Vec3ToSimxFloat(T3.second), simx_opmode_blocking);
	simxFloat temp2[3];
	temp2[0] = 1.0;
	temp2[1] = 1.0;
	temp2[2] = 1.0;
