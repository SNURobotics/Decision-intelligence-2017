...
vector<simxInt> objhandles;
vector<simxInt> jointhandles;
vector<srStateJoint*> joints;
vector<simxChar*> objnames;
vector<simxChar*> jointnames;
vector<srSystem*> systems;
...
{
	//objnames.resize(0);
	//objhandles.resize(objnames.size());
	jointnames.resize(0);
	char* str;
	char* str2;
	for (int i = 1; i < 13; i++)
	{
		...
		snprintf(str2, 3, "%d", i);
		str = strcpy(str, "robot_joint");
		str = strcat(str, str2);
		jointnames.push_back(str);
	}
		
	jointhandles.resize(jointnames.size());

	//getObjectHandle(clientID, objnames, objhandles);
	int returnCode = getObjectHandle(clientID, jointnames, jointhandles);

	joints.resize(jointnames.size());
	for (unsigned int i = 0; i < 6; i++)
	{
		joints[i] = (srStateJoint*)robot1->gJoint[i];
		joints[i + 6] = (srStateJoint*)robot2->gJoint[i];
	}
	return returnCode;
}
...
void vrepRendering()
{
	while (TRUE)
	{
		if (clientID != -1)
		{
			...
			//setVrepBaseLinksFromSrlib(clientID, systems, objhandles);
			if (isHYUPlanning)
				setVrepJointValuesFromSrlib(clientID, joints, jointhandles);
