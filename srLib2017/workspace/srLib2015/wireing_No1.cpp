...
{	
	...
	boxfortape->setBaseLinkFrame(EulerXYZ(Vec3(0, 0, 0), input));
	...
	string out_line;
	ofstream out("../../../data/environment_setting/wireing_No1_output.txt");
	for (int i = 0; i < ur3Manager->m_lowerJointLimit.size(); i++) {
		out << ur3Manager->getJointVal()[i] << endl;
	}
	out.close();
	out = ofstream("../../../data/environment_setting/wireing_rod_position.txt");
	for (int i = 0; i < 3; i++) {
		out << boxfortape->getBaseLinkFrame().GetPosition()[i] << endl;
	}
	out.close();
...
void updateFunc()
{
	...
	if (trajcnt < ur3traj1.size())
	{
		...
		if (trajcnt == ur3traj1.size() - 1) {
			ur3Manager->setGripperPosition(idlePos);
			trajcnt = -5; // Wait some time before reset
