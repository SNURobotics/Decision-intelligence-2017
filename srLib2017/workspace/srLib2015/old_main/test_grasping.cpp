...
void updateFunc()
{
	...
	//gripPos[0] = 0.0294*0.5;
	//gripPos[1] = - 0.0294*0.5;
	//rManager1->setGripperPosition(gripPos);
	
	static unsigned int step = 0;
	static int count = 0;
	static int colli_cnt = 0;
	//((srStateJoint*)joint1)->m_State.m_rCommand = -0.5;
	...
	static Eigen::VectorXd qtemp = initPos[0];
	Eigen::VectorXd tautemp(6);
	tautemp.setZero();
	tautemp[2] = 10.1;
	if (colli_cnt > 300)
		rManager1->controlJointAcc(tautemp);
	if (count == 1)
	...
	/*if (count == 1)
		qtemp.setRandom();*/
	//qtemp[2] = 0.1*step;
	//rManager1->controlJointTorque(qtemp);
	//rManager1->setJointVal(qtemp);
...
void environmentSetting()
{
	...
	for (unsigned int i = 0; i < jig.size(); i++)
	{
		...
		{
			initSE3[2 * i + j] = EulerZYX(Vec3(SR_PI*0.1*i, 0.0, 0.0), Vec3(-0.2 + 0.1*i, 0.3 + 0.1*j, 0.0));
...
void robotSetting()
{
	...
	robot1->GetBaseLink()->SetFrame(EulerZYX(Vec3(-SR_PI_HALF,0.0,0.0),Vec3(-0.0, 0.7, 0.0)));
...
void RRT_problemSetting()
{
	...
	int flag;	
	initPos.push_back(rManager1->inverseKin(allSE3_busbar[0] * Tbusbar2gripper, &robot1->gMarkerLink[Indy_Index::MLINK_GRIP], true, SE3(), flag));
	cout << initPos[0].transpose() << endl;
	for (unsigned int i = 1; i < allSE3_busbar.size(); i++)
	{
		goalPos.push_back(rManager1->inverseKin(allSE3_busbar[i] * Tbusbar2gripper, &robot1->gMarkerLink[Indy_Index::MLINK_GRIP], true, SE3(), flag));
...
void collisionTestSetting()
{
	...
	joint1->SetChildLinkFrame(SE3(Vec3(0.0, 0.0, -0.2)));
	joint1->SetActType(srJoint::TORQUE);
