...
int main(int argc, char **argv)
{
	...
	cout << robot->GetBaseLink()->GetFrame() << endl;

	rManager->setRobot(robot, robot2);
	rManager->setSpace(&gSpace, &gSpace2);
	...
	rManager->m_activeArmInfoCopy->setActiveArmInfo((srSystem*)robot2, endeffector2);
	rManager->m_activeArmInfoCopy->setHybrid();
	...
	int b = rManager->m_activeArmInfoCopy->m_numJoint;

	Eigen::VectorXd ulim(a);
	Eigen::VectorXd llim(a);
	for (int i = 0; i < a; i++)
	{
		ulim(i) = SR_PI*2.;
		llim(i) = -SR_PI*2.;
	}
	rManager->setJointLimit(ulim, llim);
	...
	time(&start);
	for (int i = 0; i < 100; i++)
		torque2 = rManager->inverseDyn2(q, dq, ddq);
	time(&end);
