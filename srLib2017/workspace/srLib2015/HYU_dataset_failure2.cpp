...
void updateFunc()
{
	...
	if (taskIdx == 2 && Norm(hctrl->T_des_trj[0].GetPosition() - busbarlink->GetPosition()) < 0.001)
	{
		taskIdx = 3;
	...
	//if (collideStarted)
	//	cnt_f++;
	//if (!des_changed && abs(targetObj->GetBaseLink()->m_ConstraintImpulse[5]) > 0.005)
	//{
	//	cnt_f++;
	//	collideStarted = true;
	//}
	//if (!des_changed && cnt_f > 50)
	//{
	//	des_changed = true;
	//	hctrl->T_des_trj[0] = jigAssem->GetBaseLink()->GetFrame() * jigAssem->holeCenter[holeNum] * Thole2busbar;
	//}

	//if (!des_changed2 && des_changed && Norm(hctrl->T_des_trj[0].GetPosition() - busbarlink->GetPosition()) < 0.001)
	//{
	//	hctrl->SelectMtx = Eigen::MatrixXd();
	//	hctrl->T_des_trj[0] = SE3(Vec3(0.0, 0.0, -0.025)) * jigAssem->GetBaseLink()->GetFrame() * jigAssem->holeCenter[holeNum] * Thole2busbar;
	//	des_changed2 = true;
	...
	if (taskIdx == 3 && !isDataSaved && abs(hctrl->T_des_trj[0].GetPosition()[2] - (hctrl->m_endeffector->GetFrame()*hctrl->m_offset).GetPosition()[2]) < 0.0005)
	{
